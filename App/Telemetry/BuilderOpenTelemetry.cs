...
namespace Telemetry
{
    public class BuilderOpenTelemetry
    {
		...
		internal static void prepareBuilder(WebApplicationBuilder builder) {
			// Configure logging
            buildLogging(builder);
            // Configure tracing and metrics
            buildTracingAndMetrics(builder);
		}

		internal static void prepareRunnerToTelemetry(WebApplication app) {
			// Enable the /metrics endpoint which will be scraped by Prometheus
            usePrometheusScraping(app);
